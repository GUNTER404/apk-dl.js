...
let apkcombo = {
  search: async function (args) {
    ...
    $('div.content-apps > a').each(function (a, b) {
      ...
      name.push(nem)
      link.push($(b).attr('href').replace('https://apkcombo-com.translate.goog/', 'https://apkcombo.com/').replace(
...
let apkdl = {
  ...
  download: async function (url) {
    ...
    $ = cheerio.load(res)
    let link = 'https:' + $(
      'body > div.mdl-layout.mdl-js-layout.mdl-layout--fixed-header > div > div > div > div > div > div:nth-child(1) > div:nth-child(3) > a').attr('href')
    return {
      img,
      developer,
      appname,
